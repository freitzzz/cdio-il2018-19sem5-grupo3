...
tspd2(Orig,L,DistanciaTotal):- todasAsCidades(NCidades), NCPorVisitar is NCidades - 1, %Começa-se por descobrir o nº de cidades (sem considerar a origem)
...
todasAsCidades(NCidades):- findall(X, city(X, _, _,_), L), length(L, NCidades). %Percorre a BC e retorna o número de cidades existente

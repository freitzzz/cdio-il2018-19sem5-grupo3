            ...
            <div v-if = "panelEditMaterial" class="modal-card" style="width: auto">
                    ...
                    <section class="modal-card-body">
                        <b-field label="Materials">
                            <b-select placeholder="Select a material" icon="tag" v-model="selectedMaterial">
                        ...
                        <b-field label="Designation" >
                         ...
                         <b-field label="Colors">
                            <b-select placeholder="Edit a color" icon="tag" v-model="selectedColor">
                            ...
                            <b-select placeholder="Edit a finish" icon="tag" v-model="selectedFinish">
  ...
  methods: {
    ...
    deleteFinish() {
      Axios.delete(
        ...
        .then(this.selectedMaterial.finishes.splice(this.selectedFinish, 1))
    ...
    deleteColor() {
      Axios.delete(
        ...
        .then(response => {
          let selectedColorIndex = this.selectedMaterial.colors.indexOf(this.selectedColor)
          this.selectedMaterial.colors.splice(selectedColorIndex, 1);
          this.selectedColor = null;
          })
        .catch(function(error) {
          alert("Oh amigo, vai dar uma curva")
        });
    },
    ...
    addColor() {  
      if (
        ...
        let color = {
          ...
          red: parseInt(this.inputColorValues.replace("#", "").substring(0, 2), 16),
          green: parseInt(this.inputColorValues.replace("#", "").substring(2, 4), 16),
          blue: parseInt(this.inputColorValues.replace("#", "").substring(4, 6), 16),
          alpha: "0"
        }

      Axios.post(
        `http://localhost:5000/mycm/api/materials/${this.selectedMaterial.id}/colors`, color)
        .then(this.selectedMaterial.colors.push(color))
        .catch(function(error) {});    
      } else {
    ...
    addFinish() {
      if (
        ...
        this.selectedMaterial.finishes.indexOf(this.inputFinishDesignation.trim()) < 0
      ) {
